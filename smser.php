...
mysql_query("CREATE TABLE IF NOT EXISTS `$set_table` (`id` INT( 11 ) NOT NULL AUTO_INCREMENT ,`token` VARCHAR( 100 ) NOT NULL ,`goad` VARCHAR( 100 ) NULL ,`insta` VARCHAR( 100 ) NULL ,`mone` INT( 11 ) NOT NULL , `smsdlimt` INT( 11 ) NOT NULL ,`unsmser` VARCHAR( 100 ) NULL ,`lim_sets` VARCHAR( 100 ) NOT NULL ,`bul_sets` VARCHAR( 100 ) NOT NULL ,`yoacc` VARCHAR( 100 ) NULL ,`twacc` VARCHAR( 100 ) NULL ,`ppem` VARCHAR( 100 ) NULL ,`smscount` INT( 11 ) NOT NULL ,`mynum` VARCHAR( 100 ) NULL, `adv_sets` VARCHAR( 10 ) ,`country` VARCHAR( 100 ) NULL ,`fbacc` VARCHAR( 100 ) ,`bannpic` VARCHAR( 200 ) ,`bannlink` VARCHAR( 200 ) ,`lang_dict` VARCHAR( 10 ) ,`mymess` VARCHAR( 100 ) NULL ,PRIMARY KEY (  `id` ) ,UNIQUE KEY  `id` (  `id` )) ENGINE = MYISAM DEFAULT CHARSET = utf8 AUTO_INCREMENT =1;");mysql_query("INSERT INTO $set_table (id, smscount, mymess, fbacc, lang_dict, adv_sets, unsmser, lim_sets, smsdlimt, bul_sets) VALUES (\"1\", \"160\", \"I like 2-waysms.com\", \"2waysmsgateway\", \"en\", \"dis\", \"diss\", \"diiss\", \"1\", \"sing\")");} else { $clmns = array("adv_sets" => "varchar( 10 ) NULL", "bannpic" => "varchar( 200 ) NULL", "bannlink" => "varchar( 200 ) NULL", "goad" => "varchar( 100 ) NULL", "insta" => "varchar( 100 ) NULL", "unsmser" => "varchar( 100 ) NULL", "mone" => "INT( 11 ) NOT NULL", "smsdlimt" => "INT( 11 ) NOT NULL", "yoacc" => "VARCHAR( 100 ) NULL", "twacc" => "VARCHAR( 100 ) NULL", "lim_sets" => "VARCHAR( 100 ) NOT NULL", "bul_sets" => "VARCHAR( 100 ) NOT NULL", "ppem" => "VARCHAR( 100 ) NULL");	foreach ($clmns as $clmnsk => $clmnsv){		mysql_query("select $clmnsk from $set_table") or mysql_query("ALTER TABLE $set_table ADD $clmnsk $clmnsv ;");	}}$wdx = "select * from $set_table where id = '1'"; 
...
if ($option == "save"){
	...
	if ($a1_code == "$b1_code") { 
		...
		$result = mysql_query("UPDATE $set_table SET lang_dict='$lang_dict', ppem='$ppem', lim_sets='$lim_sets', bul_sets='$bul_sets', insta='$insta', mone='$mone', twacc='$twacc', yoacc='$yoacc', adv_sets='$adv_sets', unsmser='$un_smser', fbacc='$fbupd', bannpic='$bannpic', goad='$goad', bannlink='$bannlink', mymess='$mymes', mynum='$mynum', smscount='$smscou', smsdlimt='$smsdlim', token='$token', country='$country' where id = '1'") or die(mysql_error());
