...
if (!$set_table_exists) { 
	mysql_query("CREATE TABLE IF NOT EXISTS `$set_table` (`id` INT( 11 ) NOT NULL AUTO_INCREMENT ,`token` VARCHAR( 100 ) NOT NULL ,`smscount` INT( 11 ) NOT NULL ,`mynum` VARCHAR( 100 ) NULL ,`country` VARCHAR( 100 ) NULL ,`fbacc` VARCHAR( 100 ) ,`bannpic` VARCHAR( 200 ) ,`bannlink` VARCHAR( 200 ) ,`lang_dict` VARCHAR( 10 ) ,`mymess` VARCHAR( 100 ) NULL ,PRIMARY KEY (  `id` ) ,UNIQUE KEY  `id` (  `id` )) ENGINE = MYISAM DEFAULT CHARSET = utf8 AUTO_INCREMENT =1;");
...
if ($option == "save"){
	...
	if ($a1_code == "$b1_code") { 
		...
		$result = mysql_query("UPDATE $set_table SET lang_dict='$lang_dict', fbacc='$fbupd', bannpic='$bannpic', bannlink='$bannlink', mymess='$mymes', mynum='$mynum', smscount='$smscou', token='$token', country='$country' where id = '1'") or die(mysql_error());
